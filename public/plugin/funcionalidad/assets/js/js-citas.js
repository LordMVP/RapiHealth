					...
					if (meses[mes-1] == data[0].periodo) {
						...
						//for (i = 1, y = 1; i <= data[0].dias; i++, y++) { 
							for  (x=14, z=0; x <= 20; x++, z=z+30) { 
								...
								var fecha_bd = new Date(ano2+'/'+mes2+'/'+dia2);
								fecha_bd = fecha_bd.yyyymmdd();

							//var horass = parseInt(hora);

							fecha_envio = ano2 + '/' + mes2 + '/' + dia2;
							//fecha_envio = "que putas pasa";
							//alert(fecha_bd + ' * ' + matriz[b][0] + ' * ' + b);


							if (fecha_bd == matriz[b][0] && matriz[b][1] == hora) {
								html += '<td>Asignado</td>'
								...
								b++;
								console.log(b);
							}else if (fecha_bd == matriz[b][0] && matriz[b][1] != hora) {
								html += '<td>Libre</td>'
